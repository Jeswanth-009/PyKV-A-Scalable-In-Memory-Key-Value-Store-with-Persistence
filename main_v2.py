...
# ── Replication config ─────────────────────────────────────────────
# Support multiple replicas: REPLICA_URLS="http://host1:8001,http://host2:8002"
# Fallback to single REPLICA_URL for backwards compatibility
_raw_urls = os.getenv("REPLICA_URLS", os.getenv("REPLICA_URL", "http://127.0.0.1:8001"))
REPLICA_URLS: list[str] = [u.strip().rstrip("/") for u in _raw_urls.split(",") if u.strip()]
IS_STANDBY = os.getenv("IS_STANDBY", "0") in ("1", "true", "True")
AUTO_RUN_INTEGRATION = os.getenv("AUTO_RUN_INTEGRATION", "0") in ("1", "true", "True")
REPLICATION_RETRIES = int(os.getenv("REPLICATION_RETRIES", "3"))
REPLICATION_TIMEOUT = float(os.getenv("REPLICATION_TIMEOUT", "5.0"))

# Track per-replica health for the status endpoint
_replica_health: dict[str, str] = {url: "unknown" for url in REPLICA_URLS}


async def _send_to_replica(client: httpx.AsyncClient, url: str, op: str, key: str, value: Any = None) -> None:
    """Send a single operation to one replica with retry + exponential backoff."""
    for attempt in range(1, REPLICATION_RETRIES + 1):
        try:
            if op == "SET":
                r = await client.post(f"{url}/keys/{key}", json={"value": value})
            elif op == "DEL":
                r = await client.delete(f"{url}/keys/{key}")
            else:
                return
            r.raise_for_status()
            _replica_health[url] = "ok"
            return
        except Exception as exc:
            _replica_health[url] = f"error: {exc}"
            if attempt < REPLICATION_RETRIES:
                await asyncio.sleep(0.3 * attempt)   # 0.3s, 0.6s backoff
            else:
                print(f"[REPLICATION] All {REPLICATION_RETRIES} attempts failed for {op} {key} → {url}: {exc}")


async def replicate_entry(op: str, key: str, value: Any = None) -> None:
    """Fan-out writes to all configured replica URLs in parallel."""
    if IS_STANDBY or not REPLICA_URLS:
        return
    async with httpx.AsyncClient(timeout=REPLICATION_TIMEOUT) as client:
        await asyncio.gather(
            *[_send_to_replica(client, url, op, key, value) for url in REPLICA_URLS],
            return_exceptions=True,
        )


async def replicate_clear() -> None:
    """Efficiently clear all replicas with a single DELETE /keys call each."""
    if IS_STANDBY or not REPLICA_URLS:
        return
    async with httpx.AsyncClient(timeout=REPLICATION_TIMEOUT) as client:
        results = await asyncio.gather(
            *[client.delete(f"{url}/keys") for url in REPLICA_URLS],
            return_exceptions=True,
        )
        for url, r in zip(REPLICA_URLS, results):
            if isinstance(r, Exception):
                _replica_health[url] = f"error: {r}"
                print(f"[REPLICATION] Clear failed for {url}: {r}")
            else:
                _replica_health[url] = "ok"
...
async def clear_all():
    ...
    for k in cleared:
        await log_del(k)
    await replicate_clear()   # single batch call to each replica
...
# ── /replication/status — monitor replica health ──────────────────
@app.get(
    "/replication/status",
    tags=["Admin"],
    summary="Check health of all configured replicas",
)
async def replication_status():
    """
    Returns enabled status and live health of every replica.
    Health is updated after each replication attempt.
    """
    if not REPLICA_URLS:
        return {"enabled": False, "replicas": []}

    # Ping all replicas in parallel for current latency
    async def ping(url: str) -> dict:
        try:
            async with httpx.AsyncClient(timeout=2.0) as client:
                r = await client.get(f"{url}/health")
                return {"url": url, "status": "ok", "response": r.json()}
        except Exception as e:
            return {"url": url, "status": "unreachable", "error": str(e)}

    results = await asyncio.gather(*[ping(url) for url in REPLICA_URLS])
    return {
        "enabled":  True,
        "is_standby": IS_STANDBY,
        "retries":  REPLICATION_RETRIES,
